...
import org.onecmdb.ui.gwt.desktop.client.widget.multi.SingleSelectValueModelCI;
...
public class CITableColumnConfig2 extends ColumnConfig implements ComponentPlugin {

	private Grid grid;
	private AttributeColumnConfig config;
	private CMDBPermissions permissions;

	public CITableColumnConfig2() {
		super();
		init();
	}
	public CITableColumnConfig2(AttributeColumnConfig config, CMDBPermissions perm) {
		super();
		this.config = config;
		this.permissions = perm;
		init();
	}
	public CITableColumnConfig2(String id, String name, int width) {
		super(id, name, width);
		init();
	}
	...
	protected Component getSelectionMenu() {
		...
		SingleSelectValueModelCI select = new SingleSelectValueModelCI(config.getMDR(), config.getAlias(), type, permissions);
		SelectContentPanel<ValueModel> selCp = new SelectContentPanel<ValueModel>("Select 1 " + config.getType(), select);
		return(selCp);
	}
	...
	protected CellEditor getCIEditor() {
		...
		AdaptableTriggerField<ValueModel> field = new AdaptableTriggerField<ValueModel>(getSelectionMenu(), "");
	
		field.setPropertyEditor(new PropertyEditor<ValueModel>() {

			public ValueModel convertStringValue(String value) {
				return null;
			}

			public String getStringValue(ValueModel value) {
				if (value == null) {
					return("");
				}
