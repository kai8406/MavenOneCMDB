...
public class ReferenceQueryTableEditor extends LayoutContainer {

	private GroupDescriptionConfig model;
	private CMDBPermissions permission;
	private Grid<BaseModel> grid;
	
	public ReferenceQueryTableEditor(CMDBPermissions perm, GroupDescriptionConfig model) {
		this.model = model;
		...
		if (permission.isEditable()) {
			...
			add.addSelectionListener(new SelectionListener<ComponentEvent>() {
				...
				public void componentSelected(ComponentEvent ce) {
					...
					m.set(GroupDescriptionConfig.REFERENCE_QUERY_MANDATORY, true);
					model.addQueryReference(m);
					store.insert(m, 0);
			...
			undo.addSelectionListener(new SelectionListener<ComponentEvent>() {
				...
				public void componentSelected(ComponentEvent ce) {
					...
					// Update model.
					model.setQueryReferences(store.getModels());
				}
			});
	...
	private List<BaseModel> buildModel() {
		return(this.model.getQueryReferences());
	}
