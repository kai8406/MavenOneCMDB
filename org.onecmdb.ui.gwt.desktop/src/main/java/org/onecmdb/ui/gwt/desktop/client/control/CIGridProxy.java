...
public class CIGridProxy extends RpcProxy<BaseListLoadConfig, BaseListLoadResult<CIModelCollection>> {

	@Override
	protected void load(BaseListLoadConfig loadConfig,
			AsyncCallback<BaseListLoadResult<CIModelCollection>> callback) {
			ModelServiceFactory.get().loadGrid(CMDBSession.get().getToken(), loadConfig, callback);
		};
}
