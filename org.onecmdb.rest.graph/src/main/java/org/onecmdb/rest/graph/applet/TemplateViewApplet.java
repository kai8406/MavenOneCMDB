...
public class TemplateViewApplet extends JApplet {
	...
	public void init() {
		...
		String rootRefCI = getParameter("rootReferenceCI");
		if (rootRefCI == null || rootRefCI.length() == 0) {
			rootRefCI = "CIReference";
		}
		...
		mainFrame = new MainTemplateBrowser(rootCI, rootRefCI);
	} catch (Throwable t) {
		destroy();
