...
/**
 * RelationExpression between a <em>source</em> RFC and a <em>target</em> Transaction. 
 *
 */
public class TransactionRelationExpression extends RelationExpression  {
	...
	public Criterion getSourceCriterion() {
		if (this.getSourceIds() != null) {
			...
			DetachedCriteria targetCrit = getSource().setProjection(Projections.property(getSourceProjection()));
		
			return(Property.forName(getRelationSourceProperty()).in(targetCrit));
		}
	}
