...
/**
 * Stores alll the attribute's for a ci.
 * <br>
 * Returned from query /template/<i>template-name</i>/attribute
 *
 */
public class AttributeCollectionContext extends AbstractCacheContext implements ICmdbObjectFactory {
	
	private ICi ci;

	public AttributeCollectionContext(Map<String, Object> context, ICi ci) {
		super(context);
		this.ci = ci;
	}
	@Override
	...
	public void newObject(String alias) {
		...
		IAttributeModifiable attributeMod = templateMod.createAttribute(alias, SimpleTypeFactory.STRING, null, 0, 1, null);
